const dbMock = {
    imoveis: [
        {
            id: 1,
            nome: 'Cidade Nova, Belo Horizonte',
            descricao: 'Apartamento de 3 quartos, com 2 suítes. Amplo espaço de 110m².',
            preco: 'R$487.000,00',
            tipo: 'Apartamento',
            imagem: 'https://imgbr.imovelwebcdn.com/avisos/22/29/78/07/84/69/1200x1200/3737544120.jpg'
        },
        {   id: 2, 
...
let db = JSON.parse (localStorage.getItem('dbImoveis'))
if (!db) {
    db = dbMock
}

function getDescricaoImoveis (id) {
    let idx = db.imoveis.findIndex (elem => elem.id == id)
    if (idx != -1)
        return db.imoveis[idx].descricao
    else    
